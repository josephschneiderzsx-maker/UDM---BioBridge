# UDM Mobile App - Version compacte (petits écrans Android)

Variante de l’app avec **fonds pleins** (sans glassmorphism) et **layout adapté aux petits écrans** (ex. Motorola 2021). Même logique et API que `../mobile-app` ; seuls l’UI et les espacements/headers ont été modifiés.

- **Fonds** : tous les écrans et la barre d’onglets utilisent des couleurs pleines (`colors.surface` / `colors.background`) au lieu de `GlassBackground`.
- **Petit écran** : si la hauteur d’écran &lt; 640px, les headers flottants sont réduits et le padding bas de contenu est diminué (voir `constants/layout.js`).
